    ...
    pub fn get_btn_def(&self, input: Option<ButtonPartial>) -> ButtonDef {
        ...
        match (&self.button_def, input) {
            (Some(internal), Some(input)) => {
                def.text = input.text.unwrap_or(internal.text.clone().unwrap_or(def.text));
                def.font = (input.font.unwrap_or(internal.font.clone().unwrap_or(def.font))).clone_weak();
                def.width = input.width.unwrap_or(internal.width.unwrap_or(def.width));
                def.height = input.height.unwrap_or(internal.height.unwrap_or(def.height));
                def.border = input.border.unwrap_or(internal.border.unwrap_or(def.border));
                def.justify_content = input.justify_content.unwrap_or(internal.justify_content.unwrap_or(def.justify_content));
                def.align_items = input.align_items.unwrap_or(internal.align_items.unwrap_or(def.align_items));
                def.border_radius = (input.border_radius.unwrap_or(internal.border_radius.unwrap_or(def.border_radius))).clone();
                def.font_size = input.font_size.unwrap_or(internal.font_size.unwrap_or(self.font_size.unwrap_or(def.font_size)));
                def.bg_color = input.bg_color.unwrap_or(internal.bg_color.unwrap_or(self.bg_color.unwrap_or(self.bg_color.unwrap_or(def.bg_color))));
                def.text_color = input.text_color.unwrap_or(internal.text_color.unwrap_or(self.text_color.unwrap_or(self.text_color.unwrap_or(def.text_color))));
                def.border_color = input.border_color.unwrap_or(internal.border_color.unwrap_or(self.border_color.unwrap_or(self.border_color.unwrap_or(def.border_color))));
            }
            (None, Some(input)) => {
...
impl<'w, 's> UIBuilder<'w, 's> {
        ...
        pub fn with_button<T: Component>(
            ...
            let b_def = internal.defaults.get_btn_def(button_def);
            internal.commands
                .entity(internal.current_entity)
                .entry::<Node>()
                .and_modify(move |mut node| {
                    node.width = b_def.width;
                    node.height = b_def.height;
                    node.border = b_def.border;
                    node.justify_content = b_def.justify_content;
                    node.align_items = b_def.align_items;
                });
            ...
            internal.commands
                ...
                .insert((
                    component,
                    Button,
                    BorderColor(b_def.border_color),
                    b_def.border_radius,
                    BackgroundColor(b_def.bg_color),
                ))
                .with_child((
                    Text::new(b_def.text),
                    TextFont::from_font(b_def.font).with_font_size(b_def.font_size),
                    TextColor(b_def.text_color),
                ));
            internal
        }

...
use bevy::prelude::{Commands, Entity, Has, Name, NextState, Query, ResMut, With, info, warn};
...
pub fn remove_surplus_population(
    ...
    for (_area_entity, name, mut area, has_city) in &mut areas {
        ...
        if area.has_surplus(has_city) {
            ...
            if has_city {
                info!(
                    "[SURPLUS] '{}': Removing all tokens from area with city",
                    name
                );
                ...
                info!(
                    "[SURPLUS] '{}': Removing surplus tokens from area with {} players",
                    name, num_players
                );
                ...
                info!(
                    "[SURPLUS] '{}': Removing surplus tokens from single-player area",
                    name
                );
                ...
                warn!(
                    "[SURPLUS] '{}': No players in area but has_surplus returned true - clearing any stale data",
                    name
                );
            }

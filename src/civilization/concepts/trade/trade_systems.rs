...
use crate::civilization::concepts::trade::trade_functions::{
    cheapest_cards_to_give, most_valuable_cards_to_request,
};
...
use crate::civilization::game_moves::{
    AcquireCivilizationCardsMove, AvailableMoves, GameMove, TradeMove,
};
    ...
    for event in recalc_player_reader.read() {
        if let Ok((player_trade_cards, player_civ_cards, treasury)) = player_query.get(event.player)
        {
            ...
            for card_name in &affordable_cards {
                ...
                moves.insert(
                    command_index,
                    GameMove::AcquireCivilizationCards(AcquireCivilizationCardsMove::AcquireCard(
                        *card_name,
                    )),
                );
            }
        ...
        if is_not_settling_trade {
            if let Ok(trading_cards) = player_cards_query.get(event.player) {
                    ...
                    .filter(|(receiver, _)| *receiver != event.player)
                {
                    ...
                    moves.insert(
                        command_index,
                        GameMove::Trade(TradeMove::ProposeTrade(receiver, matching_cards.clone())),
                    );
...
pub fn update_player_trading_status(
    ...
    let trading_set: bevy::platform::collections::HashSet<Entity> =
        trading_players.iter().collect();
            ...
            parent
                ...
                .with_children(|card| {
                    ...
                    .with_children(|row| {
                        ...
                        let offering_text: Vec<String> = offer
                            .offering_guaranteed
                            .iter()
                            .map(|(card, count)| {
                                if *count > 1 {
                    ...
                    .with_children(|row| {
                        ...
                        let wanting_text: Vec<String> = offer
                            .wanting_guaranteed
                            .iter()
                            .map(|(card, count)| {
                                if *count > 1 {
...
pub fn update_human_done_ui(
    ...
    mut create_btn_query: Query<
        &mut BackgroundColor,
        (With<CreateOfferButton>, Without<DoneTradingButton>),
    >,
...
pub fn spawn_create_offer_modal(
    ...
    let mut owned_commodities: Vec<(TradeCard, usize)> =
    ...
    // Full-screen scrim. The dialog used to be a bare 80%x90% panel, which left
    // the always-on status panels (A.S.T. bottom-left, the HUD strip) showing
    // around and behind its edges while the player was mid-offer.
    ui.insert(CreateOfferModal)
        ...
        .bg_color(Color::srgba(0.0, 0.0, 0.0, 0.75));
    ...
    ui.with_child(|dialog| {
        ...
        dialog.add_row(|header| {
            ...
            header.add_text_child(
                "Create Trade Offer",
                Some(TextStyle::size_color(22.0, Color::srgb(0.9, 0.8, 0.3))),
            );
        ...
        // ── Fixed footer: summary, validation, and the Publish button ─────
        dialog.add_column(|section| {
            section
                .width_percent(100.0)
...
pub fn handle_card_count_adjust(
    mut interaction_query: Query<(&Interaction, &CardCountAdjustButton), Changed<Interaction>>,
    mut create_offer_state: ResMut<CreateOfferState>,
) {
    for (interaction, btn) in &mut interaction_query {
        ...
        if btn.is_offering {
            let current = create_offer_state
                .offering_guaranteed
                .get(&btn.card)
                .copied()
                .unwrap_or(0);
...
pub fn handle_hidden_count_buttons(
    mut interaction_query: Query<(&Interaction, &HiddenCountButton), Changed<Interaction>>,
    mut create_offer_state: ResMut<CreateOfferState>,
) {
    for (interaction, btn) in &mut interaction_query {
        if *interaction == Interaction::Pressed {
        ...
        match *interaction {
            Interaction::Pressed => {
                *bg_color = BackgroundColor(Color::srgb(0.3, 0.7, 0.4));
                ...
                if let Some(human) = trade_ui_state.human_player
                    && let Ok(mut offer) = offers_query.get_mut(accept_btn.offer)
                    && offer.can_accept(human)
                {
                    ...
                    if let Ok(player_cards) = player_cards_query.get(human) {
                        ...
                        for (card, count) in &offer.wanting_guaranteed {
                            ...
                            if player_has < *count {
                                can_fulfill = false;
                                debug!(
                                    "Cannot accept: need {} {:?} but only have {}",
                                    count, card, player_has
                                );
                                break;
                            }
                        }
                        ...
                        let total_needed = offer.wanting_guaranteed.values().sum::<usize>()
                            + offer.wanting_hidden_count;
                        let total_have = player_cards.number_of_trade_cards();
                        if total_have < total_needed {
                            can_fulfill = false;
                            debug!(
                                "Cannot accept: need {} total cards but only have {}",
...
pub fn update_offer_summary_display(
    ...
    mut summary_query: Query<
        &mut Text,
        (With<OfferSummaryDisplay>, Without<OfferValidationDisplay>),
    >,
    mut validation_query: Query<
        (&mut Text, &mut TextColor),
        (With<OfferValidationDisplay>, Without<OfferSummaryDisplay>),
    >,
    mut publish_btn: Query<(&mut BackgroundColor, &Children), With<PublishOfferButton>>,
    mut text_color_query: Query<
        &mut TextColor,
        (
            Without<OfferValidationDisplay>,
            Without<OfferSummaryDisplay>,
        ),
    >,
) {
    // Build offering text
    let offering_text = if create_offer_state.offering_guaranteed.is_empty()
        && create_offer_state.offering_hidden_count == 0
    {
        "nothing".to_string()
    } else {
    ...
    // Build wanting text
    let wanting_text = if create_offer_state.wanting_guaranteed.is_empty()
        && create_offer_state.wanting_hidden_count == 0
    {
        "nothing".to_string()
    } else {
        let mut parts = Vec::new();
        ...
        if is_valid {
            ...
            *color = TextColor(Color::srgb(0.4, 0.8, 0.4));
            ...
            if create_offer_state.guaranteed_offering_count() != 2 {
                issues.push(format!(
                    "need 2 guaranteed offering (have {})",
                    create_offer_state.guaranteed_offering_count()
                ));
            }
            if create_offer_state.guaranteed_wanting_count() != 2 {
                issues.push(format!(
                    "need 2 guaranteed wanting (have {})",
                    create_offer_state.guaranteed_wanting_count()
                ));
            }
            if create_offer_state.total_offering() < 3 {
                issues.push(format!(
                    "need 3+ total offering (have {})",
                    create_offer_state.total_offering()
                ));
            }
            if create_offer_state.total_wanting() < 3 {
                issues.push(format!(
                    "need 3+ total wanting (have {})",
                    create_offer_state.total_wanting()
                ));
            }
    ...
    *ai_offer_timer = 0.0;
    ...
    for (ai_entity, ai_name, ai_cards, _, personality) in ai_players.iter() {
        ...
        let has_active_offer = existing_offers
            .iter()
            .any(|o| o.creator == ai_entity && !o.withdrawn && o.accepted_by.is_none());
        ...
        // A commodity set of n cards is worth face_value x n^2 (rule 28.2), so
        // what matters is not a card's face value but what it does to a set.
        // Give away whatever costs least: singletons and small sets, even of a
        // high face value, before touching a set we are building.
        let offering_guaranteed = cheapest_cards_to_give(&ai_card_counts, 2);
        let cards_offered: usize = offering_guaranteed.values().sum();

        if cards_offered < 2 {
            continue;
        }

        // Ask for whatever adds the most: the (n+1)th card of a set we already
        // hold is worth face_value x (2n+1), so reinforcing our biggest set
        // beats collecting one of everything. Still capped at our own top face
        // value -- an AI holding nothing but Ochre asking for Gold just gets
        ...
        let requestable: Vec<TradeCard> = TradeCard::iter()
            .filter(adv_civ_protocol::TradeCardTrait::is_commodity)
            .filter(|c| c.value() <= max_own_value)
            // Never ask for a card we are simultaneously offering away.
            .filter(|c| !offering_guaranteed.contains_key(c))
            .collect();

        let wanting_guaranteed = most_valuable_cards_to_request(&ai_card_counts, &requestable, 2);
        let cards_wanted: usize = wanting_guaranteed.values().sum();
...
pub fn ai_accept_trade_offers(
    ...
    for (ai_entity, ai_name, ai_cards, _, personality, ai_civ_cards) in ai_players.iter() {
        ...
        for (_offer_entity, mut offer) in &mut offers {
            ...
            let ai_commodities: HashMap<TradeCard, usize> =
                ai_cards.commodity_cards().into_iter().collect();
...
fn ai_select_settlement_cards(
    ...
    let mut available: HashMap<TradeCard, usize> =
        player_cards.commodity_cards().into_iter().collect();
    ...
    // Fill the rest with whatever costs the least to lose. Face value alone is
    // the wrong measure: a set of n cards is worth face_value x n^2, so the
    // fifth Ochre (value 1) costs 9 to give up while a lone Gem (value 5) costs
    // only 5. Break up singletons, keep the sets.
    let giving = cheapest_cards_to_give(&available, hidden_left);
    for (card, count) in giving {
        *selected.entry(card).or_insert(0) += count;
        hidden_left -= count;
    }

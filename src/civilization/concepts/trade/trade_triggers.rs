...
use crate::civilization::concepts::trade::trade_components::{
    CanTrade, PublishedOffer, PublishedOffersList, TradeButtonAction, TradeOffer,
};
use bevy::color::palettes::basic::{GREEN, YELLOW};
use bevy::log::debug;
use bevy::prelude::{
    Add, Color, Commands, Entity, JustifyContent, NextState, On, Query, Remove, Res, ResMut,
    UiRect, Val, With,
};
use lava_ui_builder::LavaTheme;
use lava_ui_builder::{TextStyle, UIBuilder};
...
pub fn offer_published(
    ...
    if let Ok((trade_offer_entity, trade_offer)) = published_offer_query.get(trigger.event().entity)
        && let Ok(ui_list) = published_offers_list.single()
    {
        let mut ui_builder =
            UIBuilder::start_from_entity(new_commands, ui_list, false, Some(ui_theme.clone()));
        ...
        ui_builder
            ...
            .display_flex()
            .flex_column()
            .size(Val::Percent(48.0), Val::Auto)
            ...
            .parent()
            .foreach_child(
                trade_offer.initiator_pays.iter(),
            ...
            .foreach_child(
                ...
                |builder: &mut UIBuilder, (commodity, count)| {
                    builder.default_text(format!("{commodity}: {count} (Guaranteed)"));
                },
            )
            .parent()
            .child()
            ...
            .parent()
            .foreach_child(
                trade_offer.initiator_gets.iter(),
                |builder: &mut UIBuilder, (commodity, count)| {
                    builder.default_text(format!("{commodity}: {count}"));
                },
            )
